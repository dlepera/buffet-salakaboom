 ...
 * @Projeto	: FrameworkDL
 * @Data	: 23/06/2014 11:41:50
 */

namespace Modelo;

class PermissoesGrupo extends Principal{
    # Propriedades desse modelo
    protected $permissao_grupo, $permissao_modulo, $permissao_ver = 0, $permissao_inserir = 0, $permissao_editar = 0,
        $permissao_remover = 0, $permissao_total = 0;
    
    public function __construct($g = null, $m = null){
        parent::__construct('dl_painel_grupos_permissoes', 'permissao_');
        
        # Query de seleção
        $this->bd_select = "SELECT"
            . " %s"
            . " FROM %s AS P"
            . " INNER JOIN dl_painel_modulos AS M ON( M.modulo_id = P.permissao_modulo )"
            . " INNER JOIN dl_painel_grupos_usuarios AS G ON( G.grupo_usuario_id = P.permissao_grupo )";
        
        if( !empty($g) && !empty($m) )
            $this->_selecionarID($g, $m);
    } // Fim do método mágico de construção da classe
    
    /**
     * Selecionar um registro desse modelo pelo ID
     * 
     * @param int $grupo: ID do grupo
     * @param int $modulo: ID do módulo
     * 
     * @return void
     */
    public function _selecionarID($grupo, $modulo){
        ...
        $this->_permissao_grupo($grupo);
        $this->_permissao_modulo($modulo);
        
        $lis_m = end($this->_listar("P.{$this->bd_prefixo}grupo = {$this->permissao_grupo} AND P.{$this->bd_prefixo}modulo = {$this->permissao_modulo}", null, 'P.*'));
    ...
    /**
     * Obter ou editar o valor da propriedade $permissao_grupo
     * 
     * @param int $valor: string contendo o valor a ser atribuído à $permissao_grupo
     * 
     * @return int: valor da propriedade $permissao_grupo
     */
    public function _permissao_grupo($valor=null){
        return is_null($valor) ?
            (int)$this->permissao_grupo
        : $this->permissao_grupo = (int)$valor;
    } // Fim do método _permissao_grupo
    ...
    protected function _salvar($salvar=true){
        if( !$this->_qtde_registros("permissao_grupo = {$this->permissao_grupo} AND permissao_modulo = {$this->permissao_modulo}") ):
            $query = "INSERT INTO ". $this->_bd_tabela() ." ("
                    . " permissao_grupo, permissao_modulo, permissao_ver, permissao_inserir, permissao_editar, permissao_remover, permissao_total) VALUES ("
                    . " {$this->permissao_grupo}, {$this->permissao_modulo}, '{$this->permissao_ver}', '{$this->permissao_inserir}', '{$this->permissao_editar}',"
            ...
            $query = "UPDATE ". $this->_bd_tabela() ." SET"
                    ...
                    . " WHERE permissao_grupo = {$this->permissao_grupo} AND permissao_modulo = {$this->permissao_modulo}";
        endif;
