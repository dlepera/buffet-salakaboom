<?php

/**
 * @Autor	: Diego Lepera
 * @E-mail	: d_lepera@hotmail.com
 * @Projeto	: FrameworkDL
 * @Data	: 19/05/2014 19:32:25
 */

# Nomes de modelos
define('TXT_MODELO_CONTATOSITE', 'contato do site');

# Títulos de páginas
define('TXT_TITULO_MENU_PRINCIPAL', 'Menu principal');
define('TXT_TITULO_PAGINA_INICIAL', 'Página inicial');
define('TXT_TITULO_INSTITUCIONAL', 'A história do Buffet Salakaboom');
define('TXT_TITULO_ALBUNS_DE_FOTOS', 'Álbuns de fotos');
define('TXT_TITULO_FALE_CONOSCO', 'Fale conosco');

# Links
define('TXT_LINK_INSTITUCIONAL', 'O Buffet');
define('TXT_LINK_ALBUNS_DE_FOTOS', 'Álbuns de fotos');
define('TXT_LINK_FALE_CONOSCO', 'Fale conosco');
define('TXT_LINK_VISUALIZAR_EMAIL_COMO_HTML', 'Mostrar e-mail como HTML');

# Descrições / labels de campos
define('TXT_LABEL_NOME_COMPLETO', 'Nome completo');
define('TXT_LABEL_EMAIL', 'E-mail');
define('TXT_LABEL_TELEFONE', 'Telefone');
define('TXT_LABEL_ASSUNTO', 'Assunto');
define('TXT_LABEL_MENSAGEM', 'Mensagem');

# Mensagens de e-mails
define('MSG_EMAIL_CASO_NAO_ESTEJA_CONSEGUINDO_VISUALIZAR', 'Caso não esteja conseguindo visualizar esse e-mail corretamente, por favor clique no link abaixo.');
define('MSG_DESENVOLVIDOR_POR', 'Site desenvolvido por Diego Lepera');

/**
 * ContatoSite
 */
# Sucesso
define('SUCESSO_CONTATOSITE_ENVIAR', 'Obrigado por entrar em contato conosco!<br />Em breve retornaremos o seu contato :)');